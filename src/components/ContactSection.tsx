...
import { Mail, Send, Github, Linkedin, MapPin, Instagram } from "lucide-react";
...
export const ContactSection: React.FC = () => {
  ...
  const { data, loading } = usePortfolioSection("contact");
  ...
  const inputClasses = cn("w-full rounded-xl border border-border/20 bg-background/20 backdrop-blur-md", "px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground/60", "focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/40", "transition-all duration-200");
      ...
      <section id="contact" className="relative w-full py-24 md:py-32 overflow-hidden">
        <div className="relative z-10 container mx-auto px-4 md:px-6 max-w-4xl">
          <motion.div custom={0} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-14">
            <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-primary/10 mb-5"><Mail className="w-7 h-7 text-primary" /></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
            <motion.form custom={1} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} onSubmit={handleSubmit}
              ...
              <div><label htmlFor="name" className="block text-sm font-mono text-muted-foreground mb-1.5">Name</label>
                <input id="name" name="name" type="text" placeholder="Your name" value={form.name} onChange={handleChange} maxLength={100} className={inputClasses} /></div>
              <div><label htmlFor="email" className="block text-sm font-mono text-muted-foreground mb-1.5">Email</label>
                <input id="email" name="email" type="email" placeholder="you@example.com" value={form.email} onChange={handleChange} maxLength={255} className={inputClasses} /></div>
              <div><label htmlFor="message" className="block text-sm font-mono text-muted-foreground mb-1.5">Message</label>
                <textarea id="message" name="message" rows={5} placeholder="Tell me about your project..." value={form.message} onChange={handleChange} maxLength={1000} className={cn(inputClasses, "resize-none")} /></div>
              <button type="submit" disabled={sending} className={cn("w-full flex items-center justify-center gap-2 rounded-xl py-3 px-6", "bg-primary text-primary-foreground font-semibold font-mono text-sm", "hover:opacity-90 transition-opacity duration-200", "disabled:opacity-50 disabled:cursor-not-allowed")}>
            ...
            <motion.div custom={2} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="md:col-span-2 space-y-6">
              <div className="rounded-2xl border border-border/20 bg-background/30 backdrop-blur-md p-6 space-y-5">
                ...
                {contactInfo.map((info) => {
                  ...
                  return (<div key={info.text} className="flex items-center gap-3">
              ...
              <div className="rounded-2xl border border-border/20 bg-background/30 backdrop-blur-md p-6 space-y-4">
                ...
                <div className="flex flex-wrap gap-3">
                  {data.socialLinks.map((link) => {
                    ...
                    return (<a key={link.label} href={link.href} target="_blank" rel="noreferrer" aria-label={link.label}
                      ...
                      <Icon size={16} /><span>{link.label}</span></a>);
                  })}
                </div>
              </div>
            </motion.div>
