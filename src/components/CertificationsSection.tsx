...
export const CertificationsSection: React.FC = () => {
  const { data, loading } = usePortfolioSection("certifications");
  if (loading || !data) return null;

  const cardItems: CardStackItem[] = data.certifications.map((cert) => ({
    id: cert.id, title: cert.title, description: cert.description, imageSrc: cert.imageSrc, tag: cert.tag,
  }));
    ...
    <section id="certifications" className="relative w-full py-24 md:py-32 overflow-hidden">
      <div className="relative z-10 container mx-auto px-4 md:px-6 max-w-5xl">
        <motion.div custom={0} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="text-center mb-16">
          <div className="inline-flex items-center justify-center w-14 h-14 rounded-full bg-primary/10 mb-5"><Award className="w-7 h-7 text-primary" /></div>
        ...
        <motion.div custom={1} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mb-16">
          <h3 className="text-xl font-semibold font-mono text-foreground mb-6 flex items-center gap-2"><GraduationCap className="w-5 h-5 text-primary" /> Education</h3>
        ...
        {cardItems.length > 0 && (
          <motion.div custom={4} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <h3 className="text-xl font-semibold font-mono text-foreground mb-6 flex items-center gap-2"><Trophy className="w-5 h-5 text-primary" /> Certifications & Achievements</h3>
