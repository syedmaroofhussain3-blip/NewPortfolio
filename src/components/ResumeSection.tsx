...
export const ResumeSection: React.FC = () => {
  const resumeData = getPortfolioData().resume;
    ...
    printWindow.document.write(`
      ...
      <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        <h2>Education</h2>
        ${resumeData.education.map((e) => `<div class="edu-item"><div class="edu-row"><strong>${e.degree}</strong><span>${e.period}</span></div><p style="font-size:12px;color:#666">${e.institution}</p><p style="font-size:12px;color:#888">${e.detail}</p></div>`).join("")}
        <h2>Skills</h2>
        <div class="skills-grid">${resumeData.skills.map((s) => `<p><strong>${s.category}:</strong> ${s.items}</p>`).join("")}</div>
        <h2>Projects</h2>
        ${resumeData.projects.map((p) => `<div class="proj-item"><strong>${p.name}</strong><p style="font-size:12px;color:#666">${p.description}</p></div>`).join("")}
      ...
      <div className="relative z-10 container mx-auto px-4 md:px-6 max-w-4xl">
        ...
        <motion.div custom={5} variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="flex justify-center">
          <button onClick={handleDownload}
            className={cn("flex items-center gap-3 rounded-full px-8 py-4", "bg-primary text-primary-foreground font-semibold font-mono text-sm", "hover:opacity-90 transition-all duration-200 hover:scale-105", "shadow-lg shadow-primary/20")}>
